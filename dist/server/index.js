...
function requireReactServerDomWebpackServer_edge_production() {
  ...
  function getReference(target, name) {
    switch (name) {
      ...
      case "then":
        ...
        var clientReference = registerClientReferenceImpl({}, target.$$id, true), proxy2 = new Proxy(clientReference, proxyHandlers$1);
        target.status = "fulfilled";
        target.value = proxy2;
        return target.then = registerClientReferenceImpl(
          function(resolve) {
            return Promise.resolve(resolve(proxy2));
          },
          target.$$id + "#then",
          false
        );
  ...
  function serializeReadableStream(request, task, stream) {
    function progress(entry) {
      if (0 === streamTask.status)
        if (entry.done)
          ...
          try {
            streamTask.model = entry.value, request.pendingChunks++, tryStreamTask(request, streamTask), enqueueFlush(request), reader.read().then(progress, error2);
          } catch (x$11) {
            error2(x$11);
          }
    }
    function error2(reason) {
      0 === streamTask.status && (request.cacheController.signal.removeEventListener("abort", abortStream), erroredTask(request, streamTask, reason), enqueueFlush(request), reader.cancel(reason).then(error2, error2));
    }
    function abortStream() {
      if (0 === streamTask.status) {
        ...
        reader.cancel(signal).then(error2, error2);
      }
    }
  ...
  function serializeAsyncIterable(request, task, iterable, iterator) {
    function progress(entry) {
      if (0 === streamTask.status)
        if (entry.done) {
          ...
          try {
            streamTask.model = entry.value, request.pendingChunks++, tryStreamTask(request, streamTask), enqueueFlush(request), iterator.next().then(progress, error2);
          } catch (x$12) {
            error2(x$12);
          }
    }
    function error2(reason) {
      0 === streamTask.status && (request.cacheController.signal.removeEventListener(
        "abort",
        abortIterable
      ), erroredTask(request, streamTask, reason), enqueueFlush(request), "function" === typeof iterator.throw && iterator.throw(reason).then(error2, error2));
    }
    function abortIterable() {
      if (0 === streamTask.status) {
        ...
        "function" === typeof iterator.throw && iterator.throw(reason).then(error2, error2);
      }
    }
    ...
    switch (wakeable.status) {
      ...
      default:
        "string" !== typeof wakeable.status && (wakeable.status = "pending", wakeable.then(
          ...
          function(error2) {
            "pending" === wakeable.status && (wakeable.status = "rejected", wakeable.reason = error2);
          }
        ));
    }
    ...
    function progress(entry) {
      if (0 === newTask.status)
        if (entry.done)
          ...
          return model.push(entry.value), reader.read().then(progress).catch(error2);
    }
    function error2(reason) {
      0 === newTask.status && (request.cacheController.signal.removeEventListener("abort", abortBlob), erroredTask(request, newTask, reason), enqueueFlush(request), reader.cancel(reason).then(error2, error2));
    }
    function abortBlob() {
      if (0 === newTask.status) {
        var signal = request.cacheController.signal;
  ...
  function logRecoverableError(request, error2) {
    ...
    try {
      ...
      var errorDigest = supportsRequestStorage ? requestStorage.run(void 0, onError, error2) : onError(error2);
    } finally {
  ...
  function fatalError(request, error2) {
    ...
    request.cacheController.abort(
      Error("The render was aborted due to a fatal error.", { cause: error2 })
    );
  }
  ...
  function erroredTask(request, task, error2) {
    task.status = 4;
    error2 = logRecoverableError(request, error2);
    emitErrorChunk(request, task.id, error2);
      ...
      try {
        ...
        if (0 < abortableTasks.size)
          if (21 === request.type)
            ...
            var error2 = void 0 === reason ? Error(
              "The render was aborted by the server without a reason."
            ) : "object" === typeof reason && null !== reason && "function" === typeof reason.then ? Error(
              "The render was aborted by the server with a promise."
            ) : reason, digest = logRecoverableError(request, error2, null), errorId = request.nextChunkId++;
            request.fatalError = errorId;
            request.pendingChunks++;
            emitErrorChunk(request, errorId, digest, error2, false, null);
            abortableTasks.forEach(function(task) {
  ...
  function rejectChunk(response, listeners, error2) {
    for (var i = 0; i < listeners.length; i++) {
      var listener = listeners[i];
      "function" === typeof listener ? listener(error2) : rejectReference(response, listener.handler, error2);
    }
  }
  function triggerErrorOnChunk(response, chunk, error2) {
    if ("pending" !== chunk.status && "blocked" !== chunk.status)
      chunk.reason.error(error2);
    else {
      var listeners = chunk.reason;
      chunk.status = "rejected";
      chunk.reason = error2;
      null !== listeners && rejectChunk(response, listeners, error2);
    }
  }
  ...
  function loadServerReference$1(response, metaData, parentObject, key) {
    function reject(error2) {
      var rejectListeners = blockedPromise.reason, erroredPromise = blockedPromise;
      erroredPromise.status = "rejected";
      erroredPromise.value = null;
      erroredPromise.reason = error2;
      null !== rejectListeners && rejectChunk(response, rejectListeners, error2);
      rejectReference(response, handler2, error2);
    }
    var id = metaData.id;
  ...
  function initializeModelChunk(chunk) {
    ...
    try {
      ...
      chunk.status = "rejected", chunk.reason = error2;
    } finally {
  ...
  function reportGlobalError(response, error2) {
    ...
    response._closedReason = error2;
    response._chunks.forEach(function(chunk) {
      "pending" === chunk.status ? triggerErrorOnChunk(response, chunk, error2) : "fulfilled" === chunk.status && null !== chunk.reason && (chunk = chunk.reason, "function" === typeof chunk.error && chunk.error(error2));
    });
  }
  ...
  function rejectReference(response, handler2, error2) {
    handler2.errored || (handler2.errored = true, handler2.value = null, handler2.reason = error2, handler2 = handler2.chunk, null !== handler2 && "blocked" === handler2.status && triggerErrorOnChunk(response, handler2, error2));
  }
  ...
  function parseTypedArray(response, reference, constructor, bytesPerElement, parentObject, parentKey, referenceArrayRoot) {
    function reject(error2) {
      if (!handler2.errored) {
        ...
        handler2.value = null;
        handler2.reason = error2;
        var chunk = handler2.chunk;
        null !== chunk && "blocked" === chunk.status && triggerErrorOnChunk(response, chunk, error2);
      }
    }
      ...
      error: function(error2) {
        if (!closed)
          for (closed = true, nextWriteIndex === buffer.length && (buffer[nextWriteIndex] = new ReactPromise(
            ...
            triggerErrorOnChunk(response, buffer[nextWriteIndex++], error2);
      }
    });
    return iterator;
  }
...
function requireReactServerDomWebpackServer_edge_development() {
  ...
  "production" !== process.env.NODE_ENV && (function() {
      ...
      switch (name) {
        ...
        case "then":
          ...
          var clientReference = registerClientReferenceImpl(
            ...
            target.$$id,
            true
          ), proxy2 = new Proxy(clientReference, proxyHandlers$1);
          target.status = "fulfilled";
          target.value = proxy2;
          return target.then = registerClientReferenceImpl(
            function(resolve) {
    ...
    function resolveClientReferenceMetadata(config2, clientReference) {
      var modulePath = clientReference.$$id, name = "", resolvedModuleData = config2[modulePath];
      if (resolvedModuleData) name = resolvedModuleData.name;
      else {
        var idx = modulePath.lastIndexOf("#");
        -1 !== idx && (name = modulePath.slice(idx + 1), resolvedModuleData = config2[modulePath.slice(0, idx)]);
        if (!resolvedModuleData)
          throw Error(
            'Could not find the module "' + modulePath + '" in the React Client Manifest. This is probably a bug in the React Server Components bundler.'
          );
    ...
    function collectStackTracePrivate(error2, structuredStackTrace) {
      error2 = [];
      for (var i = framesToSkip; i < structuredStackTrace.length; i++) {
        ...
        if (name.includes("react_stack_bottom_frame")) break;
          ...
          callSite = callSite.isAsync(), error2.push([name, "", 0, 0, 0, 0, callSite]);
        else {
      ...
      collectedStackTrace = error2;
      return "";
    }
    function collectStackTrace(error2, structuredStackTrace) {
      collectStackTracePrivate(error2, structuredStackTrace);
    ...
    function parseStackTrace(error2, skipFrames) {
      ...
      try {
        var stack = String(error2.stack);
      } finally {
        Error.prepareStackTrace = existing;
      }
      if (null !== collectedStackTrace)
        return stack = collectedStackTrace, collectedStackTrace = null, stackTraceCache.set(error2, stack), stack;
      ...
      stackTraceCache.set(error2, existing);
      return existing;
    }
    function createTemporaryReference(temporaryReferences, id) {
      var reference = Object.defineProperties(
      ...
      if (descriptor && (descriptor.configurable || descriptor.writable) && "function" === typeof descriptor.value) {
        ...
        var wrapperMethod = function() {
          ...
          if (("assert" !== methodName || !arguments[0]) && null !== request) {
            a: {
              var error2 = Error("react-stack-top-frame");
              collectedStackTrace = null;
              framesToSkip = 1;
            ...
            null != error2 && outlineComponentInfo(request, error2);
            badge = [methodName, JSCompiler_inline_result, error2, env];
        ...
        if (owner.owner || "string" !== typeof owner.name) {
          for (; owner; ) {
            ...
            if (null != ownerStack) {
              if (owner = owner.owner) {
                ...
                var error2 = ownerStack, prevPrepareStackTrace = Error.prepareStackTrace;
          ...
          if (void 0 === prefix)
            try {
              ...
              prefix = (error2 = x.stack.trim().match(/\n( *(at )?)/)) && error2[1] || "", suffix = -1 < x.stack.indexOf("\n    at") ? " (<anonymous>)" : -1 < x.stack.indexOf("@") ? "@unknown:0:0" : "";
            }
    ...
    function serializeReadableStream(request, task, stream) {
      ...
      function error2(reason) {
        0 === streamTask.status && (request.cacheController.signal.removeEventListener(
          "abort",
          abortStream
        ), erroredTask(request, streamTask, reason), enqueueFlush(request), reader.cancel(reason).then(error2, error2));
      }
      function abortStream() {
        if (0 === streamTask.status) {
          var signal = request.cacheController.signal;
    ...
    function serializeAsyncIterable(request, task, iterable, iterator) {
      function progress(entry) {
        if (0 === streamTask.status)
          if (entry.done) {
            ...
            try {
              streamTask.model = entry.value, request.pendingChunks++, tryStreamTask(request, streamTask), enqueueFlush(request), callIteratorInDEV(iterator, progress, error2);
            } catch (x$1) {
              error2(x$1);
            }
      }
      ...
      request.cacheController.signal.addEventListener("abort", abortIterable);
      callIteratorInDEV(iterator, progress, error2);
      return serializeByValueID(streamTask.id);
    }
    function emitHint(request, code, model) {
      model = stringify(model);
    ...
    function serializeServerReference(request, serverReference) {
      ...
      var id = serverReference.$$id, location = null, error2 = serverReference.$$location;
      error2 && (error2 = parseStackTrace(error2, 1), 0 < error2.length && (location = error2[0], location = [location[0], location[1], location[2], location[3]]));
    ...
    function serializeBlob(request, blob) {
      ...
      function error2(reason) {
        0 === newTask.status && (request.cacheController.signal.removeEventListener(
          "abort",
          abortBlob
        ), erroredTask(request, newTask, reason), enqueueFlush(request), reader.cancel(reason).then(error2, error2));
      }
      function abortBlob() {
        if (0 === newTask.status) {
          var signal = request.cacheController.signal;
    ...
    function logRecoverableError(request, error2, task) {
      var prevRequest = currentRequest;
      currentRequest = null;
      try {
        var onError = request.onError;
        var errorDigest = null !== task ? supportsRequestStorage ? requestStorage.run(
          ...
          onError,
          error2
        ) : callWithDebugContextInDEV(request, task, onError, error2) : supportsRequestStorage ? requestStorage.run(void 0, onError, error2) : onError(error2);
      } finally {
        currentRequest = prevRequest;
      }
      if (null != errorDigest && "string" !== typeof errorDigest)
    ...
    function fatalError(request, error2) {
      var onFatalError = request.onFatalError;
      onFatalError(error2);
      null !== request.destination ? (request.status = CLOSED, closeWithError(request.destination, error2)) : (request.status = 13, request.fatalError = error2);
      request.cacheController.abort(
        Error("The render was aborted due to a fatal error.", { cause: error2 })
      );
    }
    function serializeErrorValue(request, error2) {
      var name = "Error", env = (0, request.environmentName)();
      try {
        name = error2.name;
        var message = String(error2.message);
        var stack = filterStackTrace(request, parseStackTrace(error2, 0));
        var errorEnv = error2.environmentName;
    ...
    function emitErrorChunk(request, id, digest, error2, debug, owner) {
      var name = "Error", env = (0, request.environmentName)();
      try {
        if (error2 instanceof Error) {
          ...
          message = "object" === typeof error2 && null !== error2 ? describeObjectForErrorMessage(error2) : String(error2), stack = [];
      } catch (x) {
        message = "An error occurred but serializing the error message failed.", stack = [];
      }
      error2 = null == owner ? null : outlineComponentInfo(request, owner);
      digest = {
        digest,
        name,
        message,
    ...
    function erroredTask(request, task, error2) {
      ...
      task.status = 4;
      var digest = logRecoverableError(request, error2, task);
      emitErrorChunk(request, task.id, digest, error2, false, task.debugOwner);
      request.abortableTasks.delete(task);
      callOnAllReadyIfReady(request);
    }
    function retryTask(request, task) {
    ...
    function performWork(request) {
      ...
      try {
        ...
        logRecoverableError(request, error2, null), fatalError(request, error2);
      } finally {
        ReactSharedInternalsServer.H = prevDispatcher, currentRequest$1 = null, currentRequest = prevRequest;
      }
    }
          ...
          if (0 < abortableTasks.size)
            if (21 === request.type)
              ...
              var error2 = void 0 === reason ? Error(
                ...
                "The render was aborted by the server with a promise."
              ) : reason, digest = logRecoverableError(request, error2, null), _errorId2 = request.nextChunkId++;
              request.fatalError = _errorId2;
              request.pendingChunks++;
              emitErrorChunk(request, _errorId2, digest, error2, false, null);
              abortableTasks.forEach(function(task) {
    ...
    function startReadingFromDebugChannelReadableStream(request$jscomp$0, stream) {
      function progress(_ref) {
        ...
        else return reader.read().then(progress).catch(error2);
      }
      function error2(e) {
        abort(
          request$jscomp$0,
    ...
    var callComponent = {
      ...
      react_stack_bottom_frame: function(iterator, progress, error2) {
        iterator.next().then(progress, error2);
      }
...
function requireReactServerDomWebpackClient_edge_production() {
  ...
  function rejectChunk(listeners, error2) {
    for (var i = 0; i < listeners.length; i++) {
      var listener = listeners[i];
      "function" === typeof listener ? listener(error2) : rejectReference(listener, error2);
    }
  }
  ...
  function triggerErrorOnChunk(response, chunk, error2) {
    "pending" !== chunk.status && "blocked" !== chunk.status ? chunk.reason.error(error2) : (response = chunk.reason, chunk.status = "rejected", chunk.reason = error2, null !== response && rejectChunk(response, error2));
  }
  function createResolvedIteratorResultChunk(response, value, done) {
    return new ReactPromise(
      "resolved_model",
  ...
  function reportGlobalError(weakResponse, error2) {
    weakResponse._closed = true;
    weakResponse._closedReason = error2;
    weakResponse._chunks.forEach(function(chunk) {
      "pending" === chunk.status ? triggerErrorOnChunk(weakResponse, chunk, error2) : "fulfilled" === chunk.status && null !== chunk.reason && chunk.reason.error(error2);
    });
  }
  ...
  function fulfillReference(reference, value) {
    ...
    try {
      ...
      rejectReference(reference, error2);
      return;
    }
    handler2.deps--;
  ...
  function rejectReference(reference, error2) {
    var handler2 = reference.handler;
    reference = reference.response;
    handler2.errored || (handler2.errored = true, handler2.value = null, handler2.reason = error2, handler2 = handler2.chunk, null !== handler2 && "blocked" === handler2.status && triggerErrorOnChunk(reference, handler2, error2));
  }
  ...
  function resolveModule(response, id, model) {
    ...
    if (model = preloadModule(clientReference)) {
      ...
      model.then(
        ...
        function(error2) {
          return triggerErrorOnChunk(response, blockedChunk, error2);
        }
      );
    } else
  ...
  function resolveErrorProd() {
    ...
    error2.stack = "Error: " + error2.message;
    return error2;
  }
...
function requireReactServerDomWebpackClient_edge_development() {
  ...
  "production" !== process.env.NODE_ENV && (function() {
    ...
    function parseStackLocation(error2) {
      error2 = error2.stack;
      error2.startsWith("Error: react-stack-top-frame\n") && (error2 = error2.slice(29));
      var endOfFirst = error2.indexOf("\n");
      if (-1 !== endOfFirst) {
        var endOfSecond = error2.indexOf("\n", endOfFirst + 1);
        endOfFirst = -1 === endOfSecond ? error2.slice(endOfFirst + 1) : error2.slice(endOfFirst + 1, endOfSecond);
      } else endOfFirst = error2;
      error2 = v8FrameRegExp.exec(endOfFirst);
      if (!error2 && (error2 = jscSpiderMonkeyFrameRegExp.exec(endOfFirst), !error2))
        return null;
      endOfFirst = error2[1] || "";
      "<anonymous>" === endOfFirst && (endOfFirst = "");
      endOfSecond = error2[2] || error2[5] || "";
    ...
    function logIOInfoErrored(ioInfo, rootEnv, error2) {
      var startTime = ioInfo.start, endTime = ioInfo.end;
      if (supportsUserTiming && 0 <= endTime) {
        var description = getIODescription(error2), entryName = getIOShortName(ioInfo, description, ioInfo.env, rootEnv), debugTask = ioInfo.debugTask;
        debugTask ? (error2 = [
          [
            "rejected with",
    ...
    function triggerErrorOnChunk(response, chunk, error2) {
      if ("pending" !== chunk.status && "blocked" !== chunk.status)
        ...
        chunk.status = "rejected";
        chunk.reason = error2;
        null !== listeners && rejectChunk(listeners, error2);
      }
    }
    ...
    function initializeDebugChunk(response, chunk) {
      ...
      if (null !== debugChunk) {
        ...
        try {
          ...
          triggerErrorOnChunk(response, chunk, error2);
        }
      }
    }
    function initializeModelChunk(chunk) {
    ...
    function reportGlobalError(weakResponse, error2) {
      if (void 0 !== weakResponse.weak.deref()) {
        ...
        response._closedReason = error2;
        response._chunks.forEach(function(chunk) {
          "pending" === chunk.status ? triggerErrorOnChunk(response, chunk, error2) : "fulfilled" === chunk.status && null !== chunk.reason && chunk.reason.error(error2);
        });
    ...
    function rejectReference(reference, error2) {
      ...
      if (!handler2.errored) {
        ...
        if (null !== handler2 && "blocked" === handler2.status) {
          ...
          triggerErrorOnChunk(reference, handler2, error2);
        }
      }
    }
    ...
    function resolveErrorDev(response, errorInfo) {
      ...
      null === ownerTask ? (response = getRootTask(response, env), error2 = null != response ? response.run(error2) : error2()) : error2 = ownerTask.run(error2);
      error2.name = name;
      error2.environmentName = env;
      return error2;
    }
              ...
              for (var j = endTimeIdx - 1; j > _i6; j--) {
                ...
                if ("string" === typeof candidateInfo.name) {
                  ...
                  if (isLastComponent && "rejected" === root.status && root.reason !== response._closedReason) {
                    var componentInfo$jscomp$2 = componentInfo$jscomp$1, trackIdx$jscomp$1 = trackIdx$jscomp$0, startTime$jscomp$2 = startTime$jscomp$1, childrenEndTime$jscomp$1 = childrenEndTime$jscomp$0, error2 = root.reason;
...
function notFound() {
  const error2 = new Error("NEXT_NOT_FOUND");
  error2.digest = `${HTTP_ERROR_FALLBACK_ERROR_CODE};404`;
  throw error2;
}
if (!isServer) {
...
function requireReactJsxRuntime_reactServer_development() {
  ...
  "production" !== process.env.NODE_ENV && (function() {
    ...
    reactJsxRuntime_reactServer_development.jsx = function(type, config2, maybeKey) {
    ...
    reactJsxRuntime_reactServer_development.jsxDEV = function(type, config2, maybeKey, isStaticChildren) {
      var trackActualOwner = 1e4 > ReactSharedInternalsServer.recentlyCreatedOwnerStacks++;
      return jsxDEVImpl(
        type,
        config2,
    ...
    reactJsxRuntime_reactServer_development.jsxs = function(type, config2, maybeKey) {
      var trackActualOwner = 1e4 > ReactSharedInternalsServer.recentlyCreatedOwnerStacks++;
      return jsxDEVImpl(
        type,
        config2,
...
class MemoryCacheHandler {
  ...
  async set(key, data, ctx) {
    ...
    if (ctx) {
      const revalidate2 = ctx.cacheControl?.revalidate ?? ctx.revalidate;
      if (typeof revalidate2 === "number" && revalidate2 > 0) {
        revalidateAt = Date.now() + revalidate2 * 1e3;
      }
    }
...
let metadata$2 = {
  ...
  description: "A real Hacker News reader built to stress-test vinext — the Vite-powered Next.js runtime. Demonstrates RSC streaming, recursive async components, server actions, dynamic routes, ISR, and proxy middleware. Built by Joe Watson DEV.",
  keywords: [
    "vinext",
    "vite",
    "next.js",
  ...
  openGraph: {
    ...
    description: "A real Hacker News reader stress-testing vinext: RSC streaming, server actions, ISR, recursive async RSC, and proxy middleware. Same Next.js code, Vite runtime.",
    siteName: "vinext Demo"
  },
  twitter: {
    card: "summary_large_image",
    title: "vinext Demo — HN Reader by Joe Watson DEV",
    description: "Stress-testing vinext with a real HN reader: RSC streaming, server actions, ISR, recursive async components, and more.",
...
function RootLayout({
  ...
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("html", { lang: "en", suppressHydrationWarning: true, className: "font-sans", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
    "body",
    {
...
const error = /* @__PURE__ */ registerClientReference(() => {
...
async function getTopStories(count = 30) {
  const ids = await fetchHN("/topstories.json");
  return ids.slice(0, count);
}
async function getNewStories(count = 30) {
  const ids = await fetchHN("/newstories.json");
  return ids.slice(0, count);
}
async function getAskStories(count = 20) {
  const ids = await fetchHN("/askstories.json");
  return ids.slice(0, count);
}
async function getItem(id) {
  try {
    const item = await fetchHN(`/item/${id}.json`);
    return item ?? null;
  } catch {
    return null;
  }
...
async function getItems(ids) {
  ...
  return items.filter(
    (item) => item !== null && !item.deleted && !item.dead
  );
}
function timeAgo(unix) {
  const seconds = Math.floor(Date.now() / 1e3 - unix);
...
const revalidate$2 = 60;
async function GET(request) {
  const { searchParams } = request.nextUrl;
  const type = searchParams.get("type") ?? "top";
  const limit = Math.min(Number(searchParams.get("limit") ?? "30"), 100);
  const ids = type === "new" ? await getNewStories(limit) : type === "ask" ? await getAskStories(limit) : await getTopStories(limit);
  ...
  return Response.json(
    ...
    {
      headers: {
        "Cache-Control": `s-maxage=${revalidate$2}, stale-while-revalidate`
      }
    }
  );
}
...
function Nav() {
  ...
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("header", { className: "sticky top-0 z-50 border-b border-zinc-800 bg-zinc-950/90 backdrop-blur-sm", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "max-w-3xl mx-auto px-4 h-14 flex items-center justify-between", children: [
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center gap-6", children: [
      ...
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("nav", { className: "flex items-center gap-1", children: [
        { href: "/?tab=top", label: "Top" },
        { href: "/?tab=new", label: "New" },
        { href: "/?tab=ask", label: "Ask" }
      ].map(({ href, label }) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        Link,
        {
          href,
      ...
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs(
        Link,
        {
          ...
          children: [
            ...
            bookmarkCount > 0 && /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { className: "text-xs bg-orange-500 text-white rounded-full w-4 h-4 flex items-center justify-center font-medium", children: bookmarkCount })
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { className: "text-xs px-2 py-1 bg-violet-500/10 border border-violet-500/20 text-violet-400 rounded-full font-mono", children: "vinext" })
    ] })
  ] }) });
}
const metadata$1 = {
...
function BookmarksPage() {
  const bookmarks2 = store.getAll();
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "min-h-screen bg-zinc-950 text-zinc-100", children: [
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(Nav, {}),
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("main", { className: "max-w-3xl mx-auto px-4 py-8", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center gap-3 mb-6", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("h1", { className: "text-xl font-semibold text-white", children: "Bookmarks" }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "text-sm text-zinc-500", children: [
          bookmarks2.length,
          " saved"
        ] })
      ] }),
      bookmarks2.length === 0 ? /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "text-center py-20 text-zinc-500", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
          "svg",
          {
        ...
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("p", { className: "mb-4", children: "No bookmarks yet." }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
          Link,
          {
            href: "/",
      ...
      ] }) : /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "space-y-3", children: bookmarks2.map((item) => {
        ...
        return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
          "article",
          {
            ...
            children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "flex items-start gap-3", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex-1 min-w-0", children: [
              item.url ? /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
                "a",
                {
                  href: item.url,
                  target: "_blank",
              ...
              host && /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "ml-2 text-xs text-zinc-500", children: [
                "(",
                host,
                ")"
              ] }),
              /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center gap-3 mt-2 text-xs text-zinc-600", children: [
                /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
                  Link,
                  {
                    href: `/story/${item.id}`,
...
async function CommentTree({
  ...
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: depth > 0 ? "mt-3 border-l border-zinc-800 pl-4 ml-2" : "space-y-5", children: visible.map((comment) => {
    if (!comment) return null;
    return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: depth === 0 ? "space-y-1" : "mt-3", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center gap-2 text-xs text-zinc-500 mb-1.5", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { className: "text-zinc-300 font-medium", children: comment.by ?? "[deleted]" }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { children: "·" }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { children: timeAgo(comment.time) }),
        comment.kids && /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "text-zinc-600", children: [
          comment.kids.length,
          " repl",
          comment.kids.length === 1 ? "y" : "ies"
        ] })
      ] }),
      comment.text && /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        "div",
        {
      ...
      comment.kids && comment.kids.length > 0 && /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        react_reactServerExports.Suspense,
        {
          fallback: /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "mt-3 ml-6 text-xs text-zinc-600 animate-pulse", children: [
            "↳ loading ",
            comment.kids.length,
            " repl",
            comment.kids.length === 1 ? "y" : "ies",
          ...
          children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(CommentTree, { ids: comment.kids, depth: depth + 1 })
        }
      )
    ] }, comment.id);
  }) });
}
const BookmarkButton = /* @__PURE__ */ registerClientReference(() => {
  throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "d6422401ef32", "default");
async function generateStaticParams() {
  const ids = await getTopStories(10);
  return ids.map((id) => ({ id: String(id) }));
}
const revalidate$1 = 300;
...
async function StoryPage({
  ...
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "min-h-screen bg-zinc-950 text-zinc-100", children: [
    ...
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("main", { className: "max-w-3xl mx-auto px-4 py-8", children: [
      ...
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("article", { className: "bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-6", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-start gap-3 mb-4", children: [
          /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex-1 min-w-0", children: [
            /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("h1", { className: "text-xl font-semibold text-white leading-snug mb-2", children: story.title }),
            host && /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs(
              "a",
              {
                href: story.url,
                target: "_blank",
        ...
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-zinc-500", children: [
          /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "text-orange-400 font-semibold", children: [
            story.score,
            " pts"
          ] }),
          /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { children: [
          ...
          /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { children: timeAgo(story.time) }),
          /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { children: [
            story.descendants ?? 0,
            " comments"
          ] })
        ] }),
        story.text && /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
          "div",
          {
      ...
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("section", { children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("h2", { className: "text-sm font-medium text-zinc-400 mb-5", children: [
          story.descendants ?? 0,
          " Comments"
        ] }),
        story.kids && story.kids.length > 0 ? /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
          react_reactServerExports.Suspense,
          {
            fallback: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "space-y-4", children: Array.from({ length: 6 }).map((_, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
              "div",
              {
            ...
            children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(CommentTree, { ids: story.kids })
          }
        ) : /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("p", { className: "text-zinc-500 text-sm", children: "No comments yet." })
      ] })
    ] })
  ] });
}
...
function Loading$1() {
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "min-h-screen bg-zinc-950 text-zinc-100", children: [
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "sticky top-0 h-14 border-b border-zinc-800 bg-zinc-950/90" }),
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("main", { className: "max-w-3xl mx-auto px-4 py-8", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-4 w-32 bg-zinc-800 rounded animate-pulse mb-6" }),
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-6", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-6 bg-zinc-800 rounded animate-pulse mb-3 w-3/4" }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-4 bg-zinc-800 rounded animate-pulse mb-4 w-1/4" }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "flex gap-4", children: Array.from({ length: 4 }).map((_, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-3 w-16 bg-zinc-800 rounded animate-pulse" }, i)) })
      ] }),
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "space-y-4", children: Array.from({ length: 6 }).map((_, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        "div",
        {
          className: "h-20 bg-zinc-900 border border-zinc-800 rounded-xl animate-pulse",
          style: { opacity: 1 - i * 0.12 }
...
function StoryCard({
  ...
  const host = domain(story.url);
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("article", { className: "group bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-600 transition-colors", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex gap-3", children: [
    rank !== void 0 && /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { className: "text-zinc-600 text-sm font-mono w-6 shrink-0 pt-0.5 text-right select-none", children: rank }),
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex-1 min-w-0", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-start gap-2 mb-2", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex-1 min-w-0", children: [
          story.url ? /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
            "a",
            {
              href: story.url,
              target: "_blank",
          ...
          host && /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "ml-2 text-xs text-zinc-500 shrink-0", children: [
            "(",
            host,
            ")"
          ] })
      ...
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-zinc-500", children: [
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { className: "text-orange-400 font-medium", children: [
          story.score ?? 0,
          " pts"
        ] }),
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("span", { children: [
        ...
        /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("span", { children: timeAgo(story.time) }),
        (story.descendants ?? 0) > 0 && /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs(
          Link,
          {
            href: `/story/${story.id}`,
...
const SearchBar = /* @__PURE__ */ registerClientReference(() => {
  throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "6451b87f0574", "default");
const metadata = {
  title: "HN Reader — vinext stress test",
  description: "A Hacker News reader built to challenge vinext: RSC streaming, server actions, dynamic routes, ISR, middleware, and recursive async components."
};
const revalidate = 60;
  ...
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "min-h-screen bg-zinc-950 text-zinc-100", children: [
    ...
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("main", { className: "max-w-3xl mx-auto px-4 py-8", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        react_reactServerExports.Suspense,
        {
          fallback: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-24 bg-zinc-900 rounded-xl animate-pulse mb-6" }),
          children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(SearchBar, { currentTab: tab, currentQuery: q })
        }
      ),
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(react_reactServerExports.Suspense, { fallback: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(StoriesSkeleton, {}), children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(StoryFeed, { tab, query: q }) }, `${tab}-${q}`)
    ] })
  ] });
}
async function StoryFeed({ tab, query }) {
  const ids = tab === "new" ? await getNewStories(30) : tab === "ask" ? await getAskStories(20) : await getTopStories(30);
  const stories = await getItems(ids);
  const filtered = query ? stories.filter(
    (s) => s.title?.toLowerCase().includes(query.toLowerCase())
  ) : stories;
  const bookmarkedIds = store.ids();
  if (!filtered.length) {
    return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "text-center py-16 text-zinc-500", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("p", { children: [
      "No stories match “",
      query,
      "”"
    ] }) });
  }
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "space-y-3", children: [
    filtered.map((story, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
      StoryCard,
      {
        story,
        rank: i + 1,
    ...
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("p", { className: "text-center text-xs text-zinc-600 pt-4", children: [
      filtered.length,
      " stories · revalidates every 60s · rendered by vinext"
    ] })
  ] });
}
function StoriesSkeleton() {
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "space-y-3", children: Array.from({ length: 12 }).map((_, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
    "div",
    {
...
function Loading() {
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("div", { className: "min-h-screen bg-zinc-950 text-zinc-100", children: [
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "sticky top-0 h-14 border-b border-zinc-800 bg-zinc-950/90" }),
    /* @__PURE__ */ jsxRuntime_reactServerExports.jsxs("main", { className: "max-w-3xl mx-auto px-4 py-8 space-y-3", children: [
      /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("div", { className: "h-24 bg-zinc-900 rounded-xl animate-pulse mb-6" }),
      Array.from({ length: 12 }).map((_, i) => /* @__PURE__ */ jsxRuntime_reactServerExports.jsx(
        "div",
        {
...
function __sanitizeErrorForClient(error2) {
...
function rscOnError(error2) {
  ...
  if (process.env.NODE_ENV === "production" && error2) {
    const msg = error2 instanceof Error ? error2.message : String(error2);
    const stack = error2 instanceof Error ? error2.stack || "" : "";
...
async function renderErrorBoundaryPage(route, error2, isRscRequest, request) {
  ...
  const rawError = error2 instanceof Error ? error2 : new Error(String(error2));
...
async function _handleRequest(request, __reqCtx) {
  ...
  const middlewareFn = proxy ?? proxy;
  if (typeof middlewareFn !== "function") {
    const _fileType = "Proxy";
    const _expectedExport = "proxy";
